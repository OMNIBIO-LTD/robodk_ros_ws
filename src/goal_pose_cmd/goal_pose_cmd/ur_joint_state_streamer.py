#!/usr/bin/env python3
"""
ROS2 node that reads joint positions from a UR robot in RoboDK and publishes
them as sensor_msgs/JointState.

This is a variant of ``joint_state_streamer`` for UR robots: it publishes only
the 6 arm joints and has no gripper-slider joint or gripper command handling.

Parameters
----------
publish_rate  : float  – publishing frequency in Hz (default 50.0)
topic         : str    – output topic name (default '/joint_states')
joint_names   : str    – comma-separated joint names (default UR joint names)
...
class URJointStateStreamer(Node):
    def __init__(self):
        super().__init__('ur_joint_state_streamer')
...
def main(args=None):
    ...
    node = URJointStateStreamer()

    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        node.get_logger().info('Shutting down ur_joint_state_streamer...')
    finally:
        node.destroy_node()
        rclpy.shutdown()
