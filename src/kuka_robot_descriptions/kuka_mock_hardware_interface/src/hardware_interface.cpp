...
std::vector<hardware_interface::StateInterface> KukaMockHardwareInterface::export_state_interfaces()
{
  ...
  if (!populate_interfaces(
        info_.gpios, gpio_interfaces_, gpio_states_, state_interfaces, true))
  {
...
KukaMockHardwareInterface::export_command_interfaces()
{
  ...
  {
    if (!populate_interfaces(
          info_.sensors, sensor_interfaces_, sensor_mock_commands_,
  ...
  {
    if (!populate_interfaces(
          info_.gpios, gpio_interfaces_, gpio_mock_commands_, command_interfaces,
          true))
    {
      throw std::runtime_error(
  ...
  const std::vector<std::string> & /*stop_interfaces*/)
{
  ...
  for (const auto & key : start_interfaces)
  {
    ...
    if (joint_it_found != info_.joints.end())
    {
      ...
      if (
        key ==
        info_.joints[joint_index].name + "/" + hardware_interface::HW_IF_POSITION)
      {
        joint_found_in_x_requests_[joint_index] += 1;
      }
      if (
        key ==
        info_.joints[joint_index].name + "/" + hardware_interface::HW_IF_VELOCITY)
      {
        if (!calculate_dynamics_)
        {
          RCUTILS_LOG_WARN_NAMED(
      ...
      if (
        key ==
        info_.joints[joint_index].name + "/" + hardware_interface::HW_IF_ACCELERATION)
      {
        if (!calculate_dynamics_)
        {
          RCUTILS_LOG_WARN_NAMED(
